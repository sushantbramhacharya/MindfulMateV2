...
def register():
    name = request.form.get('name')  # <-- Added
    ...
    # Validation
    if not all([name, email, password, confirm_password, date_of_birth]):
    ...
    try:
        ...
        user_id = UserModel.create_user(name, email, password, dob)  # <-- Updated
        ...
        if user:
            ...
            response = make_response(jsonify({
                ...
                'user': {
                    'id': user['id'],
                    'email': user['email'],
                    'name': user.get('name')  # <-- Optional: expose name to frontend
                }
            }))
